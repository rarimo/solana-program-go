...
func WithdrawFTMinted(adminSeed, program, txHash, tokenSeed, eventId, networkFrom string, amount uint64, privateKey string, ownerPrivateKey string) {
	seed := getSeedFromString(adminSeed)
	...
	args := contract.WithdrawArgs{
		...
		Origin:     targetContent.Origin,
		TokenSeed:  &token,
		SignedMetadata: &contract.SignedMetadata{
			Name:     "Tether USD",
			Symbol:   "USDT",
			URI:      "",
			Decimals: 9,
